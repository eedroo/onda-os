...
export default function LeadsPage() {
  ...
  const { grupos, activeId, sensors, handleDragStart, handleDragOver, handleDragEnd } = useKanbanDnd<Lead>({
    itens: leadsFiltrados, colunas,
    getId: l => l.id!,
    getColuna: l => l.status,
    onMudarColuna: (id, novoStatus) => moverStatus(id, novoStatus as LeadStatus),
  })
  const leadAtivo = activeId ? leadsFiltrados.find(l => l.id === activeId) : null
  ...
  function renderCartao(lead: Lead) {
        ...
        <DndContext sensors={sensors} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
          <div style={{ display: 'flex', gap: 10, minWidth: 0 }}>
            {COLUNAS.map(col => {
              ...
              return (
                <DroppableColumn key={col.id} id={col.id} items={itens.map(l => l.id!)}>
                  {isOver => (
                    <div style={{ width: 226, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
                      ...
                      {itens.map(lead => (
                        <SortableItem key={lead.id} id={lead.id!}>{renderCartao(lead)}</SortableItem>
                      ))}
          ...
          <DragOverlay>{leadAtivo ? <div style={{ width: 226 }}>{renderCartao(leadAtivo)}</div> : null}</DragOverlay>
        </DndContext>
      </div>
    </div>
  )
